...
public struct CameraContainerRenderableComponent: RenderableComponent, AbstractLetterboxable {
    ...
    public func setIsUserInteractionEnabled(_ isEnabled: Bool) -> Self {
        var copy = self
    ...
    public func mask(size: CGSize, origin: CGPoint) -> Self {
        var copy = self
        copy.mask = CGRect(origin: origin, size: size)
        return copy
    }

    public func zoom(by zoom: CameraZoom) -> Self {
        var copy = self
        copy.zoom = zoom
        return copy
    }

    public func track(_ point: CGPoint) -> Self {
        var copy = self
