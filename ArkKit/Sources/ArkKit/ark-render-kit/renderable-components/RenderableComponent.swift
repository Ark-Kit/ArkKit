...
public protocol RenderableComponent: Component, Memoizable {
...
extension RenderableComponent {
    public func center(x: Double?, y: Double?) -> Self {
        var newSelf = self
    ...
    public func rotation(_ rotation: Double) -> Self {
        var newSelf = self
        newSelf.rotation = rotation
        return newSelf
    }

    public func zPosition(_ zPos: Double) -> Self {
        var newSelf = self
    ...
    public func userInteractionsEnabled(_ isEnabled: Bool) -> Self {
        var newSelf = self
        newSelf.isUserInteractionEnabled = isEnabled
        return newSelf
    }

    public func layer(_ layer: RenderLayer) -> Self {
        var newSelf = self
        newSelf.renderLayer = layer
        return newSelf
    }

    public func shouldRerender(_ shouldRerenderDelegate: @escaping ShouldRerenderDelegate) -> Self {
        var newSelf = self
        newSelf.shouldRerenderDelegate = shouldRerenderDelegate
        return newSelf
    }

    public var areValuesEqual: AreValuesEqualDelegate {
        { old, new in
...
extension RenderableComponent {
    public func hasUpdated(previous: any Memoizable) -> Bool {
