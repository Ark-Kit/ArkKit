...
class EntityManagerTests: XCTestCase {
    ...
    func testCreateAndRemoveEntity() {
        ...
        XCTAssertTrue(entityManager.getEntities(with:[]).contains(entity), "Entity should be tracked by EntityManager.")
        
        entityManager.removeEntity(entity)
        XCTAssertFalse(entityManager.getEntities(with:[]).contains(entity), "Entity should be removed from EntityManager.")
    }
    ...
    func testCreateAndRemoveEntityWithComponent() {
        ...
        entityManager.removeEntity(entity)
        XCTAssertTrue(entityManager.getEntities(with:[TestComponent.self]).count == 0, "Entity should be removed from EntityManager.")
    }
    ...
    func testRetrievingEntitiesByComponentType() {
        ...
        let entitiesWithBothComponents = entityManager.getEntities(with: [TestComponent.self, AnotherTestComponent.self])
        
        XCTAssertEqual(entitiesWithTestComponent.count, 1, "Should find 1 entity with TestComponent.")
        XCTAssertEqual(entitiesWithBothComponents.count, 0, "Should find no entities with both TestComponent and AnotherTestComponent.")
    }
    ...
    func testRetrievingEntitiesWithMultipleComponentType() {
        ...
        XCTAssertEqual(entitiesWithBothComponents.count, 1, "Should find 1 entity with TestComponent and AnotherTestComponent.")
    }
}
